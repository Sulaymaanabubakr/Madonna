...
export default function ProductDetailPage({ params }: { params: { slug: string } }) {
    ...
    useEffect(() => {
        fetch("/api/products")
            .then((r) => r.json())
            .then((data) => {
                const found = data.items.find((p: Product) => p.slug === params.slug);
                setProduct(found || null);
            })
            .finally(() => setLoading(false));
    }, [params.slug]);

    if (loading) return <div className="container mx-auto px-4 py-12"><Skeleton className="h-[600px] w-full rounded-none" /></div>;
            ...
            <div className="border-b border-zinc-100 bg-white">
                <div className="container mx-auto px-4 py-4">
                    <div className="flex items-center text-[10px] font-semibold uppercase tracking-[0.2em] text-zinc-500">
                        ...
                        <Link href="/shop" className="hover:text-[#8B2030]">SHOP</Link>
                        <ChevronRight className="mx-2 h-3 w-3" />
            ...
            <div className="container mx-auto px-4 py-10 md:py-16">
                ...
                <div className="grid gap-10 lg:grid-cols-2 lg:gap-16">
                    ...
                    <div className="flex flex-col">
                        <h1 className="font-serif text-3xl font-bold uppercase text-zinc-900 sm:text-4xl">
                            {product.name}
                        </h1>
                        ...
                        <div className="mt-6 flex flex-col gap-1 border-b border-zinc-200 pb-8">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-3xl font-black text-[#8B2030]">
                                    {formatCurrency(product.price)}
                                </span>
                            </div>
                            <p className="mt-4 text-[13px] leading-relaxed text-zinc-600">
                                {product.description || "Premium quality product from Madonna Link Express Ventures, curated for excellence."}
                            </p>
                        </div>
                        ...
                        <div className="mt-6">
                            ...
                            <span className="text-[13px] text-zinc-500 hover:text-[#8B2030] cursor-pointer">
                        ...
                        <div className="mt-8 flex items-center gap-4 border-b border-zinc-200 pb-8">
                            ...
                            <Button
                                ...
                                className="h-12 flex-1 rounded-none bg-[#222222] px-8 text-xs font-bold uppercase tracking-widest text-white hover:bg-[#8B2030]"
                            >
                        ...
                        <div className="mt-8 flex items-center gap-4">
                            ...
                            <div className="flex items-center gap-3">
                                <Share2 className="h-4 w-4 cursor-pointer text-zinc-600 hover:text-[#8B2030]" />
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="container mx-auto px-4">
                    <Tabs defaultValue="description" className="w-full">
                        <TabsList className="flex h-auto w-full justify-start rounded-none border-b border-zinc-200 bg-transparent p-0 pb-1">
                            <TabsTrigger value="description" className="rounded-none border-b-2 border-transparent px-6 py-4 text-xs font-bold uppercase tracking-widest text-zinc-500 data-[state=active]:border-[#8B2030] data-[state=active]:text-zinc-900 data-[state=active]:shadow-none">
                                Description
                            </TabsTrigger>
                            <TabsTrigger value="additional" className="rounded-none border-b-2 border-transparent px-6 py-4 text-xs font-bold uppercase tracking-widest text-zinc-500 data-[state=active]:border-[#8B2030] data-[state=active]:text-zinc-900 data-[state=active]:shadow-none">
                                Additional Information
                            </TabsTrigger>
                            <TabsTrigger value="reviews" className="rounded-none border-b-2 border-transparent px-6 py-4 text-xs font-bold uppercase tracking-widest text-zinc-500 data-[state=active]:border-[#8B2030] data-[state=active]:text-zinc-900 data-[state=active]:shadow-none">
                                Reviews (0)
                            </TabsTrigger>
                        </TabsList>
                        <div className="bg-white p-8 shadow-sm">
                            <TabsContent value="description" className="text-[13px] leading-loose text-zinc-600 mt-0">
                                {product.description || "Consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla."}
                            </TabsContent>
                            <TabsContent value="additional" className="text-[13px] leading-loose text-zinc-600 mt-0">
                                Weight: N/A <br /> Dimensions: N/A
                            </TabsContent>
                            <TabsContent value="reviews" className="text-[13px] leading-loose text-zinc-600 mt-0">
                    ...
                    <div className="mt-16">
                        <div className="mb-6 flex items-center justify-between border-b border-zinc-200 pb-2">
                            <h3 className="text-xl font-bold uppercase tracking-widest text-zinc-900">
                                Related Products
                            </h3>
                        </div>
                        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
                            {/* Note: In a real app we'd fetch actual related products. Mocking with current product for layout. */}
